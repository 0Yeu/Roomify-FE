...
export const createContactListMockData = (count: number = 20): Contact[] => new Array(count).fill(0).map(() => ({
  name: `${Faker.name.firstName()} ${Faker.name.lastName()}`,
  address: `${Faker.address.city()}, ${Faker.address.country()}`,
  jobTitle: Faker.name.jobTitle(),
}));

export const createContactSectionsMockData = (count: number = 20) => new Array(Math.round(count / 4)).fill(0).map(() => ({
